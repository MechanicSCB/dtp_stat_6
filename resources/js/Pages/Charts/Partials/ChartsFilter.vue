    ...
    <div class="mt-6 flex text-xs mb-10 flex flex-wrap gap-x-2 gap-y-2">
        ...
        <div class="w-[180px]">
            <multiselect
                ...
                :options="filterFormData['allCategories'].map(option => option.id)"
                :custom-label="opt => filterFormData['allCategories'].find(x => x.id == opt).name"
                :multiple="true"
                :close-on-select="false"
        ...
        <div class="w-[180px]">
            <multiselect
                ...
                :options="filterFormData['allLightConditions'].map(option => option.id)"
                :custom-label="opt => filterFormData['allLightConditions'].find(x => x.id == opt).name"
                :multiple="true"
                :close-on-select="false"
                :clear-on-select="false"
                :preserve-search="true"
        ...
        <div v-if="form.regions.length" class="w-[200px]">
            <multiselect
                ...
                :options="filterFormData['allSubregions'].filter(i => form.regions.includes(i.region_id)).map(v => v.id)"
                :custom-label="opt => filterFormData['allSubregions'].find(x => x.id == opt).name"
                :multiple="true"
                :close-on-select="false"
                :clear-on-select="true"
                :preserve-search="true"
